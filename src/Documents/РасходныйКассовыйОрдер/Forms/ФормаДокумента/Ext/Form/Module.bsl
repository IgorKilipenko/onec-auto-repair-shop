...
Процедура ПриОткрытии(_)
    ...
    поляИзменяющиеТипФормы.Добавить(ЭтотОбъект.Элементы.Получатель);
    поляИзменяющиеТипФормы.Добавить(ЭтотОбъект.Элементы.ВидОперации);
...
Процедура ВидОперацииПриИзменении(_)
    изменитьТипФормы(ЭтотОбъект.Элементы.ВидОперации);
КонецПроцедуры

&НаКлиенте
Процедура ПолучательПриИзменении(_)
    изменитьТипФормы(ЭтотОбъект.Элементы.Получатель);
КонецПроцедуры
...
Процедура установитьНастройкиПоляПолучатель(Знач типПолучателя, Знач типКонтрагента = Неопределено)
    ...
    ДиагностикаКлиентСервер.Утверждение(
        типКонтрагента = Неопределено
            ИЛИ типКонтрагента = _ТипыКонтрагентов.Покупатель
    ...
    ЭтотОбъект.Элементы.Получатель.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
    ...
    ЭтотОбъект.Элементы.Получатель.ОграничениеТипа = описаниеТипаПолучателя;
    ...
    Если типПолучателя = Тип("СправочникСсылка.Контрагенты") И типКонтрагента <> Неопределено Тогда
        ...
        ЭтотОбъект.Элементы.Получатель.ПараметрыВыбора = Новый ФиксированныйМассив(параметрыВыбораПолучателя);
    КонецЕсли;
КонецПроцедуры
...
Процедура применитьНастройкиПолейФормы(Знач настройкиПолей)
    ...
    Если настройкиПолей.ТипПолучателя <> Неопределено И ЭтотОбъект.Элементы.Получатель.Видимость Тогда
        типПолучателя = ?(настройкиПолей.ТипПолучателя = _ТипыКонтрагентов.Покупатель
        ...
        типКонтрагента = Неопределено;
        Если типПолучателя = _ТипыКонтрагентов.Покупатель ИЛИ типПолучателя = _ТипыКонтрагентов.Поставщик Тогда
    ...
    ИначеЕсли НЕ ЭтотОбъект.Элементы.Получатель.Видимость Тогда // Очистить фильтры Получателя
        ЭтотОбъект.Элементы.Получатель.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
        ЭтотОбъект.Элементы.Получатель.ОграничениеТипа = Новый ОписаниеТипов(Новый Массив);
    КонецЕсли;
...
Функция проверитьДопустимоеУправляющееПоле(управляющееПоле)
    Если управляющееПоле = ЭтотОбъект.Элементы.ВидОперации
        ИЛИ управляющееПоле = ЭтотОбъект.Элементы.Получатель Тогда
...
Функция получитьНастройкиОтображенияПолей(Знач управляющееПоле)
    ...
    ИначеЕсли управляющееПоле = ЭтотОбъект.Элементы.Получатель Тогда
...
Функция получитьСкрываемыеПоляПриИзмененииВидаОперации()
    ДиагностикаКлиентСервер.Утверждение(ЗначениеЗаполнено(ЭтотОбъект.Объект.ВидОперации)
        И проверитьЭтоДопустимыйВидОперации(ЭтотОбъект.Объект.ВидОперации),
    ...
    Возврат создатьОбъектПараметровНастраиваемогоПоля(
        ЭтотОбъект.Элементы.ВидОперации, скрываемыеПоля, изменяемыеПоля, типПолучателя);
КонецФункции
...
Функция получитьСкрываемыеПоляПриИзмененииПолучателя()
    ...
    Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Получатель) И (НЕ этотКонтрагентПоставщик(ЭтотОбъект.Объект.Получатель)) Тогда
        скрываемыеПоля.Вставить(ЭтотОбъект.Элементы.Договор.Имя, ЭтотОбъект.Элементы.Договор);
    КонецЕсли;

    Возврат создатьОбъектПараметровНастраиваемогоПоля(ЭтотОбъект.Элементы.Получатель, скрываемыеПоля, изменяемыеПоля);
КонецФункции
...
//  - Неопределено - Если тип Плательщика не определен для вида операции.
&НаКлиенте
Функция определитьТипПолучателяПоВидуОперации()
    типКонтрагента = Неопределено;

    Если этоВыдачаДенегПодотчетнику() ИЛИ этоВыдачаЗаработнойПлаты() Тогда
    ...
    ИначеЕсли этоВозвратДенегПокупателю() Тогда
        типКонтрагента = _ТипыКонтрагентов.Покупатель;

    Иначе
        ДиагностикаКлиентСервер.Утверждение(Ложь, СтрШаблон(
                "Недопустимый вид операции.
                |Выбранный вид операции - ""%1"" не предусмотрен программной логикой.",
                Строка(ЭтотОбъект.Объект.ВидаОперации)));
    КонецЕсли;
...
Функция этоОплатаПоставщику()
    Возврат ЭтотОбъект.Объект.ВидОперации = _ВидыОперацийРасходаДенег.ОплатаПоставщику;
КонецФункции
...
Функция этоВозвратДенегПокупателю()
    Возврат ЭтотОбъект.Объект.ВидОперации = _ВидыОперацийРасходаДенег.ВозвратПокупателю;
КонецФункции
...
Функция этоВыдачаДенегПодотчетнику()
    Возврат ЭтотОбъект.Объект.ВидОперации = _ВидыОперацийРасходаДенег.ВыдачаПодотчетнику;
КонецФункции

// Возвращаемое значение:
//	- Булево
&НаКлиенте
Функция этоВыдачаЗаработнойПлаты()
    Возврат ЭтотОбъект.Объект.ВидОперации = _ВидыОперацийРасходаДенег.ВыдачаЗаработнойПлаты;
КонецФункции
...
Процедура инициализация()
    ...
    _Состояние.Получатель = ЭтотОбъект.Объект.Получатель;
КонецПроцедуры

// Возвращаемое значение:
//	- ФиксированнаяСтруктура
//      * ВидыОперацийРасходаДенег - ФиксированнаяСтруктура
//      * ТипыКонтрагентов - ФиксированнаяСтруктура
&НаСервереБезКонтекста
Функция получитьЗначенияПеречислений()
    результат = Новый Структура;
    результат.Вставить("ВидыОперацийРасходаДенег",
        РаботаСМетаданными.ПолучитьЗначенияПеречисления(Тип("ПеречислениеСсылка.ВидыОперацийРасходаДенег")));
    результат.Вставить("ТипыКонтрагентов",
