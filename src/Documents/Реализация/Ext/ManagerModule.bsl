...
Процедура Печать(ТабДок, Ссылка) Экспорт
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Подвал.Параметры.Заполнить(Новый Структура("СуммаДокументаПрописью",
                ЧислоПрописью(Выборка.СуммаДокумента,
...
// Устарела. Необходимо исключить обработку поля `СрокГодности` когда учет себестоимости выполняется по средней
//
// Дополняет таблицу номенклатуры данными по остаткам на складах и статьям затрат для партий номенклатуры
//  и добавляет итоги по партиям товаров (с упорядочиванием по сроку годности).
//  Для корректной работы менеджер временных таблиц должен содержать `ВТ_ТоварыДокумента`
//
//  ВНИМАНИЕ!
//  Если в учетной политике установлен учет себестоимости продаж `по средней цене` и при этом товары на складах учтены
//  по партиям (с указанием сроков годности) алгоритм списания остатков по полученным данным отработает неправильно.
//  В таком случае при изменении учетной политики необходимо выполнить корректировку
//  учтенных товаров и материалов на складах (обнулить срок годности).
//
// Параметры:
//  менеджер - МенеджерВременныхТаблиц - Менеджер должен содержать таблицу `ВТ_ТоварыДокумента` (товары документа)
//  период - Дата, МоментВремени, Граница
//  склад - СправочникСсылка.Склады, Неопределено - Если **Константы.ИспользоватьСклады** = Ложь значение игнорируется
//  выгрузить - Булево - По умолчанию = Истина
// Возвращаемое значение:
//  - ВыборкаИзРезультатаЗапроса - Если `выгрузить` = Ложь
//  - ДеревоЗначений из СтрокаДереваЗначений - Если `выгрузить` = Истина
//      * Номенклатура - СправочникСсылка.Номенклатура
...
//          ** СуммаВДокументе - Число - Стоимость партии номенклатуры в документе реализации
//          ** КоличествоОстаток - Число - Количество остатков партии номенклатуры на складе
...
Функция ПолучитьОстаткиИСтатьиЗатратДляТаблицыТоваров(
        Знач менеджер, Знач период, Знач склад = Неопределено, Знач выгрузить = Истина) Экспорт

    запрос = Новый Запрос;
    запрос.МенеджерВременныхТаблиц = менеджер;
    запрос.Текст =
        "ВЫБРАТЬ // Остатки партий товаров на складах
        ...
        |ПОМЕСТИТЬ ВТ_ОстаткиПартийНоменклатуры
        |ИЗ
        ...
        |   ВТ_ТоварыДокумента.Номенклатура.СчетБухгалтерскогоУчета КАК СчетБухгалтерскогоУчета,
        |	ВТ_ТоварыДокумента.Количество КАК КоличествоВДокументе,
        |	ВТ_ТоварыДокумента.Сумма КАК СуммаВДокументе,
        |	ВТ_ОстаткиПартийНоменклатуры.СрокГодности КАК СрокГодности,
        |	ЕСТЬNULL(ВТ_ОстаткиПартийНоменклатуры.КоличествоОстаток, 0) КАК КоличествоОстаток,
        |	ЕСТЬNULL(ВТ_ОстаткиПартийНоменклатуры.СуммаОстаток, 0) КАК СуммаОстаток
        |ИЗ
        |	ВТ_ТоварыДокумента КАК ВТ_ТоварыДокумента
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОстаткиПартийНоменклатуры КАК ВТ_ОстаткиПартийНоменклатуры
        |		ПО ВТ_ТоварыДокумента.Номенклатура = ВТ_ОстаткиПартийНоменклатуры.Номенклатура
        |
        |УПОРЯДОЧИТЬ ПО
        |	ВТ_ОстаткиПартийНоменклатуры.СрокГодности
        |ИТОГИ
    ...
    Если склад = Неопределено ИЛИ Константы.ИспользоватьСклады.Получить() = Ложь Тогда
        // Лучше использовать регулярные выражения но как оказалось
        //  1с в.24 не умеет работать с конструкциями ретроспективной проверки
