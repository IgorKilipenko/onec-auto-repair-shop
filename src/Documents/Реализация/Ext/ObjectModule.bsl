...
Функция выполнитьВсеДвиженияДокумента()
    ...
    Иначе
        списокНоменклатуры = выполнитьВсеДвиженияПоСкладам();
    КонецЕсли;
...
Функция выполнитьКонтрольОстатковПоСкладам(Знач списокНоменклатуры)
    результат = Новый Структура("Успех, СообщенияОбОшибках", Ложь);

    моментВремениДокумента = Новый Граница(ЭтотОбъект.МоментВремени(), ВидГраницы.Включая);
    остаткиПоНоменклатуре = РегистрыНакопления.ТоварыНаСкладах.ПолучитьОстаткиНоменклатуры(
            списокНоменклатуры, моментВремениДокумента, Истина);

    результат.СообщенияОбОшибках = Новый Массив;
    Для Каждого остатокНоменклатуры Из остаткиПоНоменклатуре Цикл
    ...
    Если результат.СообщенияОбОшибках.Количество() = 0 Тогда
        ...
        результат.СообщенияОбОшибках = Неопределено;
    КонецЕсли;
