...
Процедура УслугиНоменклатураПриИзменении(_)
    текущиеДанныеУслуги = ЭтотОбъект.Элементы.Услуги.ТекущиеДанные;
    текущиеДанныеУслуги.СтатьяЗатрат = получитьСтатьюЗатратНаСервере(текущиеДанныеУслуги.Номенклатура);
КонецПроцедуры
...
Функция получитьСтатьюЗатратНаСервере(Знач номенклатураСсылка)
    структураСтатьиЗатрат = РаботаСРеквизитами.ЗначенияРеквизитовОбъекта(номенклатураСсылка, "СтатьяЗатрат");
    Возврат ?(структураСтатьиЗатрат.СтатьяЗатрат <> Неопределено,
        структураСтатьиЗатрат.СтатьяЗатрат,
