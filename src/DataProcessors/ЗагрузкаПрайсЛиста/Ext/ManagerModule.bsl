...
Функция ПолучитьТаблицуЗначенийИзТабличногоДокумента(
    ...
    результат.Колонки.Добавить("Артикул", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(9)));
    ...
    Если перваяСтрока = данныеПрайсЛиста.ОбластьЗаголовков.Низ Тогда
    ...
    Для номерСтроки = перваяСтрока По последняяСтрока Цикл
        ...
        новаяСтрокаПрайса.Артикул = артикулНоменклатуры;
...
Функция найтиПозициюСтолбцовПрайсЛиста(Знач табДокПрайсЛиста)
    ...
    искомыеСтолбцы.Вставить("Артикул", Новый Структура(искомыеСтолбцы_Ключи, "(?i)\b(?:Код|Артикул)\b", ""));
    искомыеСтолбцы.Вставить("Цена", Новый Структура(искомыеСтолбцы_Ключи, "(?i)\b(?:Цен[аы]|Стоимость)\b", ""));
